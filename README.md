...
2. Для того, чтобы указать максимальное количество удаляемых элементов из Базы данных, надо задать значение следующей переменной в yaml файле:
...
3. Для того, чтобы изменить время ожидания для выполнения sql запроса Delete, следует изменить любую из двух следующих переменных:
    ...
    * `EveryTwelveHoursOfTheNightRate: "0 0 0 * * *" - Каждый раз в 12 ночи.`
    ...
    Для того, чтобы изменить значение, вы можете воспользоваться следующими настройками:
    
    1. `"* * * * * *" - Каждую секунду`
    2. `"0 * * * * *" - Каждую минуту`
    3. `"0 0 * * * *" - Каждый час`
    4. `"0 0 0 * * *" - Каждый день, в полночь`
